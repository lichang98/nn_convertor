...
def fcn_param_normalization(model: keras.Model, dataX: np.array) -> keras.Model:
    previous_factor = 1
    beg_layer_idx = 0
...
def test_fcn_ann(model: keras.models.Model, dataX: np.array, dataY: np.array):
...
if __name__ == "__main__":
    ...
    normed_model = fcn_param_normalization(model, dataX[-50:])
    test_fcn_ann(normed_model, dataX[:100], dataY[:100])
